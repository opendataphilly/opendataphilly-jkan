...
then
    ...
    then
        ...
        curl "${1}" | jq -r '.result | .[]' > working_files/all_dataset_names.txt

        # Get the datasets and filter out failed requests
        # shellcheck disable=SC2002
        cat working_files/all_dataset_names.txt | xargs -d"\n" --replace='{}' curl -fw "\n" 'https://www.opendataphilly.org/api/action/package_show?id={}&include_datasets=False&include_password_hash=True' > working_files/odp_datasets.txt
